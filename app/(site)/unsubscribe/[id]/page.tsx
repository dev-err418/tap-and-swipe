...
// Path-style entry: /unsubscribe/<contactId>
// Plunk's List-Unsubscribe regex matches `unsubscribe/<UUID>"` so this URL
// shape causes Plunk to auto-inject the L-U header (pointing to its own
// hosted page) on top of our visible branded link.
export default async function UnsubscribeByIdPage({
  params,
}: {
  ...
  return (
    <section className="flex flex-1 flex-col items-center justify-center px-6 py-12">
      <div className="w-full max-w-xl">
        ...
        <UnsubscribeCard contactId={id ?? null} />
      </div>
    </section>
  );
}
