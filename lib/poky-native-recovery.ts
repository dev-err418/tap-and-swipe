...
export function pokyNativeRecoveryExperiment(attributes: PaywallAttribute[], events: Outcome[], countries: Map<string, string>, start: number, end: number, asOf = Date.now()): MobileAppExperiment {
  ...
  return { id: "poky-native-recovery-holdout", title: "Recovery A/B test", subtitle: "Native · all proceeds after first eligibility, regardless of purchase placement", variants, scoreMetrics: ["appu_d7", "appu_d14", "appu_d30"], showUsers: true, showInstalls: false, showDownloadPaid: false };
}
